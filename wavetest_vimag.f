...
C****************************************************************************
c to run
c f77 -o exe  -fno-second-underscore -L/usr/local/netcdf/liblcs -I/usr/local/netcdf/includelcs -O wavetest_imag.f -lnetcdf 
      PROGRAM wavetest
...
c      IMPLICIT none
      ...
      PARAMETER (n=8895,m=1,dt=0.1667,s0=dt)
...
c      PARAMETER (n=5367,dt=1D0,s0=dt)
      ...
      INTEGER status,ncid        
      INTEGER periodid, timeid , waveid,waveimagid, coiid,kmax
      INTEGER perioddim, timedim, dime1(4),iindex(4)
      INTEGER londim, latdim, imax ,jmax ,lonid,latid
      DOUBLE PRECISION lon, lat ,ncoi(m,n),wavedat,waveimagdat     
...
C     s0=2dt (for Morlet) and "jtot" from Eqn(10) Torrence&Compo(1998).
      ...
      DOUBLE COMPLEX wave(n,jtot),nwave(m,n,jtot),nwaveimag(m,n,jtot)
...
c     &      recon_vari
        ...
        DO 440 j = 1,jtot
          ...
          DO 460 i=1,n
           ...
           ELSEIF(j.EQ.1)THEN
         nwaveimag(jj,i,j) = imagpart(wave(i,j))
           ELSE 
          nwaveimag(jj,i,j) = imagpart(wave(i,j))
           ENDIF     
460       CONTINUE
...
C####### period
      status = nf_def_var(ncid,'period',nf_double,1,perioddim,periodid)
...
c      status=nf_put_att_double(ncid,coiid,'_fillvalue',nf_double
c     &,1,miss)
c      if (status.ne.nf_noerr) stop 'error with coiatt3' 

C waveimag 

      status=nf_def_var(ncid,'waveimag',nf_float,3,dime1,waveimagid)
...
C write data
      ...
      DO 600  l=1, n
        ...
        DO 610  k=1,jtot
         ...
         DO 620 j=1,m
          iindex(2)=j      
           waveimagdat = nwaveimag(j,l,k)     
         status = nf_put_var1_double(ncid,waveimagid,iindex,waveimagdat)
